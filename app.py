...
# setting up the page configuration (title, icon, and layout)
...
try:
    ...
    st.error("Model file not found or not loadable. Please check that 'rf_model.joblib' is in the app folder.")
...
with col2:
    ...
    flat_model = st.selectbox(
        "Flat Model", sorted([
            "Improved", "New Generation", "Model A", "Simplified", "Premium Apartment", "Maisonette", 
    ...
    try:
        ...
        # Ensure 'month' column is parsed as datetime
        if 'month' in df_hdb.columns:
            df_hdb['month'] = pd.to_datetime(df_hdb['month'], errors='coerce')
            # Filter for selected town and get 5 most recent
            recent = (
            ...
            if not recent.empty:
                with st.expander(f"🕵️‍♂️ Most Recent Transactions in {town}", expanded=False):
                    ...
                    # Add Flat Model to displayed columns, remove date, price with commas
                    recent_display = (
...
if st.button("🔮 Predict My HDB Price", use_container_width=True):
    ...
    <div class="warning-box">
        ...
        <p>This is an AI-powered predicted estimate based on historical data. Actual prices may vary due to:
        <ul>
...
<div class="about-section">
    ...
    <p>
        Whether you’re thinking of selling, dreaming of an upgrade, or just curious, this tool is built to make things a little easier. It uses smart machine learning trained on thousands of HDB transactions to give you an estimate that reflects real market trends.
    </p>
    <p>
    ...
    <p>
        Remember, things like renovations or a great view can make a difference. So treat this estimate as a friendly guide, but always chat with HDB or a trusted agent for serious decisions.
    </p>
</div>
""", unsafe_allow_html=True)
